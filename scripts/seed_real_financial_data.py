...
from app.core.clickhouse_conn import init as init_clickhouse, close as close_clickhouse, _clickhouse_connection_manager
...
async def _fetch_existing_lookup_ids(session, table_name: str, id_column: str, name_column: str) -> Dict[str, int]:
...
async def _ensure_lookup_entries(
    ...
    for enum_value in enum_values:
        ...
        if name not in existing_ids:
            kwargs = build_kwargs_fn(enum_value) if build_kwargs_fn else {"description": f"{name} {table_name.replace('_lookup', '')}"}
            await _create_lookup_table_entry(session, factory_class, name, existing_ids, **kwargs)
...
async def create_asset_classes(session) -> Dict[str, int]:
    ...
    return await _ensure_lookup_entries(
        ...
        list(assetClassEnum),
        existing,
        lambda e: {"asset_class_name": e.value, "description": f"{e.value} asset class"}
    )
...
async def create_sub_asset_classes(session, asset_classes: Dict[str, int]) -> Dict[str, int]:
...
async def create_structure_types(session) -> Dict[str, int]:
    ...
    return await _ensure_lookup_entries(
        ...
        existing,
        lambda e: {"structure_type_name": e.value, "description": f"{e.value} structure type"}
    )
...
async def create_market_segments(session) -> Dict[str, int]:
    ...
    return await _ensure_lookup_entries(
        ...
        existing,
        lambda e: {"market_segment_name": e.value, "description": f"{e.value} market segment"}
    )
...
def build_series_from_combination(
    ...
    try:
        ...
        return metaSeriesFactory.build(
            ...
            sub_asset_class_id=lookup_maps["sub_asset_classes"].get(combo["sub_asset_class"].value),
            product_type_id=lookup_maps["product_types"].get(combo["product_type"].value),
            data_type_id=lookup_maps["data_types"].get(combo["data_type"].value),
            structure_type_id=lookup_maps["structure_types"].get(structureTypeEnum.OUTRIGHT.value),
            market_segment_id=lookup_maps["market_segments"].get(combo["market_segment"].value),
            ticker=ticker,
...
async def create_meta_series(session, lookup_maps: Dict, num_series: int = 200) -> List[metaSeries]:
    ...
    if len(all_combinations) > num_series:
        ...
        while len(combinations) < num_series:
            additional = random.sample(all_combinations, min(len(all_combinations), num_series - len(combinations)))
...
async def create_clickhouse_value_data(
    ...
    logger.info(f"💹 Creating ClickHouse value data entries ({num_per_series} per series)...")
    ...
    if not _clickhouse_connection_manager.is_initialized() or _clickhouse_connection_manager.client is None:
        logger.warning("ClickHouse not initialized or not available. Skipping value_data seeding.")
        return 0
    ...
    for series in meta_series_list:
        for i in range(num_per_series):
            ...
            if len(current_batch) >= batch_size:
                ...
                batch_count += 1
                logger.info(f"Inserted batch {batch_count}: {len(current_batch)} entries (Total: {total_count})")
                current_batch = []
    ...
    if current_batch:
        await _insert_batch_clickhouse(crud_ch, session, current_batch)
        batch_count += 1
        logger.info(f"Inserted final batch {batch_count}: {len(current_batch)} entries (Total: {total_count})")
    ...
    try:
        async with get_session_context() as session:
            ...
            if clickhouse_available:
                ...
                logger.warning("Skipping value_data creation - ClickHouse not available")
                total_value_data = 0
            ...
            logger.info(f"   - Sub Asset Classes: {len(lookup_maps['sub_asset_classes'])}")
            ...
            logger.info(f"   - Value Data: {total_value_data} (ClickHouse: {'✅' if clickhouse_available else '❌'})")
    finally:
