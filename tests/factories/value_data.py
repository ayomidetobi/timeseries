...
class ValueDataFactory(factory.Factory):
    ...
    value = factory.LazyAttribute(lambda x: Decimal(str(fake.pyfloat(left_digits=7, right_digits=8, positive=True))))
    ...
    derived_flag = factory.LazyAttribute(lambda x: fake.word().upper() if fake.boolean(chance_of_getting_true=30) else None)
