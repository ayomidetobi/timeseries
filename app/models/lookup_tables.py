...
class assetClassLookup(SQLModel, table=True):
    ...
    __table_args__ = (Index("ix_asset_class_lookup_name", "asset_class_name"),)
    ...
    sub_asset_classes: list["subAssetClassLookup"] = Relationship(
...
class productTypeLookup(SQLModel, table=True):
    ...
    __table_args__ = (Index("ix_product_type_lookup_name", "product_type_name"),)
    ...
    is_derived: bool = Field(
        default=False, description="Whether this product type represents derived data"
    )
...
class subAssetClassLookup(SQLModel, table=True):
    ...
    asset_class: Optional["assetClassLookup"] = Relationship(
        back_populates="sub_asset_classes"
    )
...
class dataTypeLookup(SQLModel, table=True):
    ...
    __table_args__ = (Index("ix_data_type_lookup_name", "data_type_name"),)
...
class structureTypeLookup(SQLModel, table=True):
    ...
    __table_args__ = (Index("ix_structure_type_lookup_name", "structure_type_name"),)
...
class marketSegmentLookup(SQLModel, table=True):
    ...
    __table_args__ = (Index("ix_market_segment_lookup_name", "market_segment_name"),)
...
class fieldTypeLookup(SQLModel, table=True):
    ...
    __table_args__ = (Index("ix_field_type_lookup_name", "field_type_name"),)
...
class tickerSourceLookup(SQLModel, table=True):
    """Ticker source lookup table."""

    __tablename__ = "ticker_source_lookup"
    __table_args__ = (
        Index("ix_ticker_source_lookup_name", "ticker_source_name"),
        Index("ix_ticker_source_lookup_code", "ticker_source_code"),
    )

    ticker_source_id: Optional[int] = Field(default=None, primary_key=True)
    ticker_source_name: str = Field(
        index=True,
        max_length=255,
        unique=True,
        description="Source name (e.g., Bloomberg, Hawkeye, RAMP, LSEG)",
    )
    ticker_source_code: Optional[str] = Field(
        default=None,
        index=True,
        max_length=100,
        description="Short code/abbreviation for the source",
    )
    description: Optional[str] = None
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(
    ...
    # Relationships
    meta_series: list["metaSeries"] = Relationship(back_populates="ticker_source")
