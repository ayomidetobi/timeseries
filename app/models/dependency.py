...
class calculationLog(SQLModel, table=True):
    ...
    __table_args__ = (
        # Composite indexes for common filter combinations
        Index(
            "ix_calculation_series_status", "derived_series_id", "calculation_status"
        ),
        Index(
            "ix_calculation_status_method", "calculation_status", "calculation_method"
        ),
        Index(
            "ix_calculation_series_method", "derived_series_id", "calculation_method"
        ),
    )
    ...
    input_series_ids: Optional[list[int]] = Field(
        default=None, sa_column=Column(ARRAY(Integer))
    )
    calculation_parameters: Optional[dict] = Field(
